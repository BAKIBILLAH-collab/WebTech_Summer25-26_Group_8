...
<form action="borrow_history.php" method="get">
    <table class="form-table"><tr>
        ...
        <td class="label"><label for="from_date">From Date:</label></td><td><input type="date" name="from_date" id="from_date" value="<?= htmlspecialchars((string) $fromDate, ENT_QUOTES, 'UTF-8') ?>"></td>
        <td class="label"><label for="to_date">To Date:</label></td><td><input type="date" name="to_date" id="to_date" value="<?= htmlspecialchars((string) $toDate, ENT_QUOTES, 'UTF-8') ?>"></td>
...
<div class="table-wrap content-section"><table class="form-table book-table">
    ...
    <?php foreach ($history as $number => $loan): ?>
        ...
        <tr><td><?= $number + 1 ?></td><td><?= htmlspecialchars($loan['title'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars($loan['author'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars($loan['category'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars($loan['borrow_date'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) ($loan['return_date'] ?? '-'), ENT_QUOTES, 'UTF-8') ?></td><td><?= number_format((float) $loan['fine'], 2) ?></td><td><span class="badge <?= $loanStatus === 'Returned' ? 'badge-success' : ($loanStatus === 'Overdue' ? 'badge-danger' : 'badge-warning') ?>"><?= $loanStatus ?></span></td></tr>
    <?php endforeach; ?>
